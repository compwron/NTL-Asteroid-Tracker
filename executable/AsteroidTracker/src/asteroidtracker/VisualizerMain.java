...
static final String DEFAULT_OUTPUT = "output.txt";
static final String DEFAULT_ALGO = "/Algorithm/Psyho.exe";

static Process solution = null;
    ...
    static public void main(String[] args) throws FileNotFoundException, IOException, ClassNotFoundException {
        ...
        options.addOption("exec", "e", true, "Sets the command you would use to execute your solution. The default value is ./Algorithm/Psyho.exe "
                          + "If your compiled solution is an executable file, the command will just be the full path to it.");
        options.addOption("testcase", "s", true, "Sets a test case number. The default value is " + DEFAULT_SEED + ".");
	options.addOption("output", "o", true, "Sets an output file. The default is " + DEFAULT_OUTPUT + ".");
	...
	String outputFile=DEFAULT_OUTPUT;
        ...
        try {
            ...
            try {
                ...
                seed = Integer.parseInt(cmd.getOptionValue("testcase", Integer.toString(DEFAULT_SEED)));
		...
		outputFile = cmd.getOptionValue("output", DEFAULT_OUTPUT);
            }
...
*/
            ...
            String execCommand = DEFAULT_ALGO;
            if (cmd.hasOption("exec")) {
                execCommand = cmd.getOptionValue("exec");
            }

            try {
                solution = Runtime.getRuntime().exec(System.getProperty("user.dir")+execCommand);
            }
            catch (IOException e) {
        ...
        testCase.generate("Testcases/testcase" + seed + ".txt");

        IAsteroidTracker asteroidTracker = new IoAsteroidTracker(solution);

        VisualizerMain vis = new VisualizerMain(testCase, asteroidTracker, isVisualizing, isPaused, simulationSpeed, outputFile);
        vis.run();
    }
